...
int    iniMotorPower = 50;     //40; //43; //; //43; //40;  //45; //55; //80; 
int    adjL         =  45;     //38; //45;  //ok for 70;  39; ok for 55//ok for power = 50 //54 for power = 100
 
float    Kp   =  0.0450;   //0.040;  //0.050;  //0.070; //0.10;  //0.0325;   //.275   //11; //3.5; //4.0;   //1.8;   //2.0; //2.4; ~//2.2; ok//2; //.75; lo//2.5; //1.0; //2; // 1.3;  //1.60;//1.4; //2.2; //1.6; //1.5; //2.0; //1.70; 

float    Kd   =  0.0450;   //34.5; //38; sucks//42; sucks//39;sucked //38; //45; //35; //40;//41; neither great //~42; ok r, //43; tracing R ok slow//44;   //40;    //60; // 80;  //50;  //47; 

float    Ki   =  0.00;     //.75; //.5;       //.25; //.5; //1.5;  //1.5;  //this parameter sucks dick! //2;  //10; //0;            //.2;  
...
void readAnalogSensors()
{
  ...
  error = (2500 - rPosition);  // russian dude's math is poor - and coding even worse!
  
  numSensors = digitize(a1Val) + digitize(a2Val) + digitize(a3Val) + digitize(a4Val) + digitize(a5Val);
  ...
  Serial.print(digitize(a1Val));
  Serial.print(" : ");
  Serial.print(digitize(a2Val));
  Serial.print(" : ");
  Serial.print(digitize(a3Val));
  Serial.print(" : ");
  Serial.print(digitize(a4Val));
  Serial.print(" : ");
  Serial.print(digitize(a5Val));
  Serial.println(""); 
