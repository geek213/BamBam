...
const int buttonTwo = 3;      // the number of the LED pin
...
void varySpeed(int startSpeed, int endSpeed)
{
  ...
  if (endSpeed > startSpeed) {    // accelerate to endSpeed
    ...
    {
      analogWrite(enA, i);  
      analogWrite(enB, i+43);  // fine-tune LEFT wheels with a little more power
      delay(20);
    }
  }
...
void loop() {
  ...
  driveForward(4000,100);     // duration (in milliseconds) and speed (0 to 255) 
